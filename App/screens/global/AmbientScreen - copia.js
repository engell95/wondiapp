...
class AmbientClass extends Component {
  ...
  Wondi(bestprice){
    if (bestprice.item.data && bestprice.item.data.length) {
      return(
        <Block>
          <ImageBackground
            style={{zIndex: -1,height:design.height / 3.6 + design.statusbar, marginBottom:design.width * 0.02}}
            imageStyle={{ borderRadius: design.height * 0.30 /8}}
            source={fondo}
          >
            {/*Productos con el mejor precio*/}
            <Block flex left style={{marginLeft:design.width * 0.05,marginTop:design.height * 0.04}}>
